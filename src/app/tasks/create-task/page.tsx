...
export default function CreateTaskPage() {
  ...
  if (theme === "dark") {
    ...
    return (
      <main className={`min-h-screen flex items-center justify-center ${mainBg} p-4`}>
        <div className={`w-full max-w-3xl ${cardBg} rounded-2xl shadow-2xl p-10 flex flex-row gap-12 animate-fade-in-up`}>
          <div className="flex-1 flex flex-col justify-center">
            <h1 className={`text-2xl font-mono font-bold mb-8 ${titleColor} animate-fade-in`}>Créer une tâche</h1>
            <form onSubmit={handleSubmit} className="flex flex-row gap-4 items-end">
              <div className="flex flex-col flex-1">
                ...
                {error && <div className="text-red-400 text-xs font-medium animate-shake mt-2">{error}</div>}
              </div>
  ...
  if (theme === "pastel") {
    ...
    return (
      <main className={`min-h-screen ${mainBg} flex flex-col items-center p-0 pt-12`}>
        <div className={`w-full max-w-lg ${cardBg} rounded-3xl shadow-xl p-8 mt-0 animate-fade-in-up`}>
          <h1 className={`text-3xl font-serif font-bold mb-8 ${titleColor} text-center animate-fade-in`}>Nouvelle tâche</h1>
        </div>
        <form onSubmit={handleSubmit} className="relative -mt-8 w-full max-w-lg flex flex-col gap-6 items-center bg-white/60 backdrop-blur-lg rounded-3xl shadow-lg p-8 animate-fade-in-up">
          <input
            type="text"
            value={title}
            onChange={(e) => { setTitle(e.target.value); setError(""); }}
          ...
          {error && <div className="text-pink-500 text-base font-medium animate-shake">{error}</div>}
  ...
  return (
    <main className={`min-h-screen flex items-center justify-center ${mainBg} p-4`}>
      <div className={`w-full max-w-md ${cardBg} rounded-xl shadow-lg p-8 animate-fade-in-up`}>
        <h1 className={`text-xl font-semibold mb-6 ${titleColor} text-center animate-fade-in`}>Créer une nouvelle tâche</h1>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input
            ...
            autoFocus
          />
          <label className="text-gray-700 text-sm font-medium animate-fade-in">Statut</label>
          <select
            value={completed ? "done" : "pending"}
            onChange={e => setCompleted(e.target.value === "done")}
            className="border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 text-base text-gray-800 transition-all duration-200 focus:scale-[1.03] focus:shadow-lg"
          ...
          {error && <div className="text-red-600 text-sm font-medium animate-shake">{error}</div>}
